...
public class Asteroids extends Application  {
   ...
   //create instance variable for the game
    ...
    int ASTEROIDS = 20;
    int currentNumAsteroids = ASTEROIDS;
	...
	public void setShipColor(Color colour) {
		shipColor = colour;
	}
	...
	/**
	 * @return the currentNumAsteroids
	 */
	public int getCurrentNumAsteroids() {
		return currentNumAsteroids;
	}
...
//Create update method to update the game 
	public void update() {

    	ObservableList<Node> content = ((Group)pane.getRoot()).getChildren(); 
    	content.clear();  
    	
    	Text livesDisplay = new Text(10,10,"Lives Remaining:  "+numLives);
    	livesDisplay.setFill(Color.WHITE);
    	content.add(livesDisplay);
    	
    	double x=(endTime-System.currentTimeMillis())/1000.0 ;
    	if(x<0)
    		x=0;
    	timeleft=(int) Math.ceil(x);

    	Text timetext = new Text(10,30,"Time Left:  "+timeleft+"s");
...
//main method
 ...
 //setup method to detect if the game is end
		keyFrame = new KeyFrame(Duration.millis(duration), new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent arg0) {
				
				if (currentNumAsteroids==0&&numLives>0) {
					gameUpdate();
			 		timeline.stop();
					stage.close();
					MissionAccomplished mission = new MissionAccomplished();
					mission.setStats(timeleft, numLives);
					try {
						mission.start(stage);
					} catch (Exception e) {
				...
				else if (numLives <= 0 && timeleft>0) {
					timeline.stop();
					stage.close();
					GameOver gameover = new GameOver();
					gameover.setStats(timeleft, numLives, ASTEROIDS,currentNumAsteroids);
					try {
